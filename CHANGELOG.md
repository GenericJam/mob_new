...
- **Fresh Android native builds failed against mob 0.8.x** (MOB-196). mob's
  `mob_nif.zig` calls libc `open`/`write`/`close` since MOB-180 (mob 0.8.2),
  and the generated `android/app/src/main/jni/build.zig` compiled it without
  `link_libc`, so `mix mob.deploy --native --android` on any app generated by
  0.5.0 stopped with "dependency on libc must be explicitly specified". The
  Zig-object helper now declares it. Existing apps: add `.link_libc = true,`
  to `addZigObject`'s `createModule` call in that file, or regenerate it.
